...
char channel_name[NICK_LEN]; // on stocke dans cette variable le nom du salon 
...
int initMsgStruct(struct message *msgstruct, char *buff){
    ...
    if (buff[0]=='/'){
        ...
        if (strcmp(command,"/nick")==0){
            ...
            strcpy(msgstruct->infos,new_buff);
            strcpy(msgstruct->receiver,new_buff);
        }
        else if (msgstruct->type == FILE_REQUEST && (!strcmp(buff,"Y\n") || !strcmp(buff,"y\n") )){
            msgstruct->type = FILE_ACCEPT;
        }
        else if (msgstruct->type == FILE_REQUEST && (!strcmp(buff,"N\n") || !strcmp(buff,"n\n") )){
        ...
        if (msgstruct->type == FILE_REQUEST && (!strcmp(buff,"Y\n") || !strcmp(buff,"y\n") )){
            msgstruct->type = FILE_ACCEPT;
        }
        else if (msgstruct->type == FILE_REQUEST && (!strcmp(buff,"N\n") || !strcmp(buff,"n\n") )){
            msgstruct->type = FILE_REJECT;
    ...
    while (1) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (strlen(msgstruct.nick_sender) > 0){
                ...
                if (msgstruct.type == ECHO_SEND || msgstruct.type == NICKNAME_LIST || msgstruct.type == NICKNAME_INFOS || msgstruct.type == MULTICAST_LIST || msgstruct.type == MULTICAST_SEND || msgstruct.type == MULTICAST_CREATE || msgstruct.type == MULTICAST_JOIN || msgstruct.type == MULTICAST_QUIT ){
                    ...
                    sprintf(payload,"\"%s:%s\"",SERV_ADDR,CLIENT_PORT);
                    if (send(sockfd, payload,  strlen(payload), 0) <= 0) {
                        break;
                    }
                    sfd_client = handle_bind_client(CLIENT_PORT);
                    if ((listen(sfd_client, SOMAXCONN)) != 0) {
                        perror("listen()\n");
                        exit(EXIT_FAILURE);
                    }
                    fds[2].fd = sfd_client;
        ...
        if (fds[1].revents & POLLIN) {
            ...
            if(msgstruct.type == FILE_ACCEPT){
                if (strcmp(msgstruct.receiver,msgstruct.nick_sender)!=0){
                    printf("Connecting to %s and sending the file...\n",msgstruct.receiver);

                    sockfd2 = handle_connect_client(CLIENT_ADDR,CLIENT_PORT);

                    sendFile(sockfd2, filename,&msgstruct);
        ...
        if (fds[2].revents & POLLIN && strcmp(msgstruct.receiver,msgstruct.nick_sender)==0){
            char client_nickname[NICK_LEN];
            strcpy(client_nickname,msgstruct.nick_sender);
            int new_fd = accept(fds[2].fd,(struct sockaddr*)&client_addr,&len);
			if (new_fd == -1){
				perror("accept\n");
				exit(EXIT_FAILURE);
			}
            if (recv(new_fd, &msgstruct, sizeof(msgstruct), 0) <= 0) {
                break;
            }
            //filename
            char destination[NICK_LEN];
            snprintf(destination,2*NICK_LEN,"%s_inbox",msgstruct.receiver);

            createDirectory(destination);

            strcat(destination,"/");
            strcat(destination,msgstruct.infos);

            receiveFile(new_fd, destination);
            printf("%s connected, sockfd = %d\nReceiving file from %s ...\n",msgstruct.nick_sender,new_fd,msgstruct.nick_sender);
            printf("\"%s\" is saved in directory \"%s\"\n",msgstruct.infos,msgstruct.receiver);
            if (clientInChannel){
                printf("\n[%s] > ",channel_name);
            }
            else{ 
            ...
            char buff2[MSG_LEN];
            sprintf(buff2,"ACK message (success) intended for :%s\n",msgstruct.nick_sender);
            ...
            if (send(sockfd, buff2, MSG_LEN, 0) <= 0) {
			    break;
		    }
