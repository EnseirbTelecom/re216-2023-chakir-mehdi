...
void echo_server(int sockfd, struct Client *client_list) {
	...
	while (1) {
        ...
        if (msgstruct.type == NICKNAME_NEW) {
			...
			if (!nicknameExists(client_list,msgstruct.infos)) {
				snprintf(to_send, 2*MSG_LEN, "User \"%s\" does not exist\n", msgstruct.infos);
			} else if (strcmp(msgstruct.nick_sender,msgstruct.infos) == 0) {
				sprintf(to_send, "You can't send a file to yourself !\n");
			} else {
				usr = findClientBy_nickname(msgstruct.infos, client_list);
				current = findClientBy_fd(sockfd,client_list);
				sock = usr->sockfd;
				snprintf(to_send, 2*MSG_LEN, "%s wants you to accept the transfer of the file named \"%s\". Do you accept? [Y/N]\n", current->nickname, buff);			
			...
			if (msgstruct.type == FILE_ACCEPT){
				snprintf(to_send, 2*MSG_LEN, "%s accepted the file transfer.\nInfos : %s", current->nickname,buff);
			}
			if (msgstruct.type == FILE_REJECT){
				snprintf(to_send, 2*MSG_LEN, "%s declined the file transfer.\nInfos : %s", current->nickname,buff);
			}
			...
			char inform[MSG_LEN];
			sprintf(inform, "Decision sent to client \"%s\"\n", usr->nickname);
			if (send(sockfd, inform, MSG_LEN, 0) <= 0) {
				break;
			}
			...
			usr = findClientBy_nickname(name, client_list);

			sock = usr->sockfd;

			sprintf(to_send,"ACK message (success) : %s has received the file \"%s\".\n",msgstruct.nick_sender,msgstruct.infos);
